...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	protected function getBasketItemsRawArray()
	{
		...
		while ($basketItem = $basketItemsResult->fetch())
		{
			...
			// TODO: MDA MEDUSA
			if (($basketItem['CAN_BUY'] !== 'Y' && $basketItem['DELAY'] !== 'Y') || !MultiShop::isAvailable($basketItem['PRODUCT_XML_ID']))
			{
	...
	protected function getBasketTotal()
	{
		...
		else
		{
			...
			$basketInfo = $this->getBasketInfo($basket);

			$basketPrice = $basketInfo['PRICE'];
			$basketWeight = $basketInfo['WEIGHT'];
			$basketBasePrice = $basketInfo['BASE_PRICE'];
			$basketVatSum = $basketInfo['VAT_SUM'];
     ...
     */
	protected function getBasketInfo(Basket $basket): array
    {
        $basketInfo = [
            'PRICE' => 0,
            'WEIGHT' => 0,
        ...
        foreach ($basket->getBasketItems() as $basketItem) {
            ...
            if (MultiShop::isAvailable($xmlId)) {
                ...
                $basketInfo['PRICE'] += $basketItem->getPrice() * $itemQuantity;
                $basketInfo['WEIGHT'] += $basketItem->getWeight() * $itemQuantity;
                $basketInfo['BASE_PRICE'] += $basketItem->getBasePrice() * $itemQuantity;
                $basketInfo['VAT_SUM'] += $basketItem->getVat();
            }
//            else {
...
//                    $basketItems[$key]['CHECK_MAX_QUANTITY'] = $check;
					$basketItems[$key]['AVAILABLE_QUANTITY'] = MultiShop::getProductStocks($basketItems[$key]['PRODUCT_XML_ID']);
					$basketItems[$key]['CHECK_MAX_QUANTITY'] = 'Y';
				}
